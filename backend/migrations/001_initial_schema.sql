-- Tidal Power Fitness Database Initialization Script (Migration 001)
-- PostgreSQL 14+

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create ENUM types (IF NOT EXISTS workaround)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS body_focus_areas (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name VARCHAR(50) UNIQUE NOT NULL,
    description TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE IF NOT EXISTS template_body_focus (
    template_id UUID NOT NULL REFERENCES workout_templates(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_exercises_workout_type ON exercises(workout_type_id);
...
CREATE INDEX IF NOT EXISTS idx_workout_templates_type ON workout_templates(workout_type_id);
CREATE INDEX IF NOT EXISTS idx_template_exercises_template ON template_exercises(template_id);
CREATE INDEX IF NOT EXISTS idx_template_exercises_order ON template_exercises(template_id, order_in_template);
...
CREATE INDEX IF NOT EXISTS idx_session_participants_client ON session_participants(client_id);
CREATE INDEX IF NOT EXISTS idx_session_exercises_session ON session_exercises(session_id);
CREATE INDEX IF NOT EXISTS idx_session_exercises_order ON session_exercises(session_id, order_in_session);
...
CREATE INDEX IF NOT EXISTS idx_exercise_logs_date ON exercise_logs(logged_at);
CREATE INDEX IF NOT EXISTS idx_progress_metrics_client ON client_progress_metrics(client_id);
CREATE INDEX IF NOT EXISTS idx_progress_metrics_date ON client_progress_metrics(log_date);
...
-- Create triggers to automatically update updated_at (DROP IF EXISTS first to avoid duplication)
