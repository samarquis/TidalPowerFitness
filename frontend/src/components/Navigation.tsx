                                        ...
                                        {managementOpen && (
                                            <div className="absolute right-0 mt-2 w-56 bg-card dark:bg-[#1a1a1a] border border-card-border rounded-lg shadow-2xl py-2 z-50 text-foreground ring-1 ring-black/10">
                                                ...
                                                <div className="px-4 py-2 grid grid-cols-2 gap-1">
                                                    <button onClick={() => { spoofRole('admin'); setManagementOpen(false); }} className={`text-[10px] px-2 py-1.5 rounded font-bold uppercase transition-all ${user?.roles?.[0] === 'admin' ? 'bg-purple-600 text-white' : 'bg-purple-600/10 text-purple-400 hover:bg-purple-600/30'}`}>Admin</button>
                                                    <button onClick={() => { spoofRole('trainer'); setManagementOpen(false); }} className={`text-[10px] px-2 py-1.5 rounded font-bold uppercase transition-all ${user?.roles?.[0] === 'trainer' ? 'bg-blue-600 text-white' : 'bg-blue-600/10 text-blue-400 hover:bg-blue-600/30'}`}>Trainer</button>
                                                    <button onClick={() => { spoofRole('client'); setManagementOpen(false); }} className={`text-[10px] px-2 py-1.5 rounded font-bold uppercase transition-all ${user?.roles?.[0] === 'client' ? 'bg-green-600 text-white' : 'bg-green-600/10 text-green-400 hover:bg-green-600/30'}`}>Client</button>
